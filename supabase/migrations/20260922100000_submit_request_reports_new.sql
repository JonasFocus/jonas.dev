-- Report whether the submission was stored now or is a retry, so only new inquiries raise alerts.
drop function public.submit_request(jsonb,text,text,text);
...
begin
  ...
  if found then
    ...
    return jsonb_build_object('id',rid,'created',false);
  end if;
  ...
  return jsonb_build_object('id',rid,'created',true);
end;
$$;
